...
fn main() {
    ...
    for line in actions.lines().map(extract_action) {
        let mut moving_value = Vec::new();
        for _ in 0..line.0 {
            let val = cols[line.1 as usize - 1].pop().unwrap();
            moving_value.push(val);
        }
        moving_value.reverse();
        for value in moving_value.into_iter() {
            cols[line.2 as usize - 1].push(value);
        }
    }
    let result = cols
